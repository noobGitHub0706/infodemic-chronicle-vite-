...
import { getRandomReaction } from '../../data/narrative';
import { PostReaction } from '../systems/PostReaction';
import { NPCReplyFeed } from '../systems/NPCReplyFeed';
import { ComboSystem, StreakBadge } from '../systems/ComboSystem';
import { CRITICISM_EVENTS } from '../../data/criticismEvents';

/**
 * 汎用レトリック識別訓練コンポーネント
 *
 * @param {Array} questions - 問題配列（ラッパー側でshuffleしてから渡すこと）
 * @param {'detailed'|'moderate'|'minimal'} feedbackLevel
 *   - detailed: 反駁フェーズあり + explanation全文
 *   - moderate: 技法タグ + explanation先頭文のみ
 *   - minimal: 技法タグのみ（explanationなし）
 * @param {Array} questionIntros - 問題前のDialogueオブジェクト配列
 * @param {{ correct, incorrect, correctClear, falseFlag }} reactions - ランダム選択用配列
 * @param {string} stageLabel - ヘッダー表示用ラベル
 * @param {boolean} isBoss - ボスモード（ヘッダー強調表示）
 * @param {Function} onTechniqueIdentified - (technique) => void 技法識別コールバック
 * @param {Function} onComplete - 完了コールバック
 */

const RebuttalPostPreview = ({ account, accountId, text }) => {
    ...
    return (
        <div className="mb-6">
            ...
            {isLongText && (
                ...
                >
                    {isExpanded ? (
                        <><span>▲</span><span>折りたたむ</span></>
                    ) : (
                        <><span>▼</span><span>全文を表示</span></>
                    )}
                </button>
            )}
        </div>
...
const PROBLEM_OPTIONS = [
    ...
    { id: 'fabricated_evidence', label: 'データや統計を捏造・歪曲している', icon: '🔬' },
    { id: 'testimonial', label: '個人の体験談を一般化している', icon: '💬' },
    { id: 'social_proof', label: '「みんながやっている」という同調圧力', icon: '🌊' },
];

// 判定結果からreactionTypeを決定
const getReactionType = (correct, hasTechnique, userSaidHasTechnique) => {
    if (correct && hasTechnique) return 'flaggedCorrectly';
    if (correct && !hasTechnique) return 'cleared';
    if (!correct && !userSaidHasTechnique) return 'missed';
    return 'falseFlagged'; // !correct && userSaidHasTechnique
};

export const RhetoricQuiz = ({
    questions,
    ...
    const [postReactionType, setPostReactionType] = useState(null);
    const [postReactionData, setPostReactionData] = useState(null);
    const [npcReplies, setNpcReplies] = useState(null);

    // 応答時間追跡
    const questionStartTimeRef = useRef(Date.now());
    const firstDecisionTimeRef = useRef(null); // handleFirstAnswer 呼び出し時点の経過ms

    // 批判イベント関連
    const [criticismEvent, setCriticismEvent] = useState(null);
    const [criticismPhase, setCriticismPhase] = useState('idle'); // 'idle' | 'showing' | 'responded'
    const [criticismChoice, setCriticismChoice] = useState(null);
    const [criticismTriggered, setCriticismTriggered] = useState(false); // 1章あたり最大1回

    const currentQuestion = questions[currentIndex];
    const hasTechnique = currentQuestion.techniques.length > 0;

    const getQuestionIntro = (index) => {
        if (!questionIntros || questionIntros.length === 0) return null;
        return questionIntros[Math.min(index, questionIntros.length - 1)];
    };

    // feedbackフェーズへの共通遷移処理
    const enterFeedback = (answer, reactionType) => {
        const reactionData = currentQuestion.reactions?.[reactionType] || null;
        const replies = reactionData?.npcReplies || null;

        setPostReactionType(reactionType);
        setPostReactionData(reactionData);
        setNpcReplies(replies);
        setAnswers(prev => [...prev, answer]);
        setLastAnswer(answer);
        setPhase('feedback');

        // 批判イベント判定: 操作的投稿を正しくフラグした後のみ発生（1章内最大1回）
        if (reactionType === 'flaggedCorrectly' && !criticismTriggered) {
            const probability = chapterNumber === 2 ? 0.3 : chapterNumber === 3 ? 0.5 : 0;
            if (probability > 0 && Math.random() < probability) {
                const events = chapterNumber === 2 ? CRITICISM_EVENTS.chapter2 : CRITICISM_EVENTS.chapter3;
                const selected = events[Math.floor(Math.random() * events.length)];
                setCriticismEvent(selected);
                setCriticismPhase('showing');
                setCriticismTriggered(true);
            }
        }
    };
    ...
    const handleFirstAnswer = (userSaidHasTechnique) => {
        ...
        firstDecisionTimeRef.current = responseTimeMs;

        if (userSaidHasTechnique && hasTechnique && feedbackLevel === 'detailed') {
            setPhase('rebuttal');
            return;
        }
        ...
        const answer = {
            ...
            isManipulative: hasTechnique,
            userJudgment: userSaidHasTechnique,
        ...
        if (reactions) {
            if (isCorrect) {
                reaction = hasTechnique
                    ? getRandomReaction(reactions.correct)
                    : getRandomReaction(reactions.correctClear);
            } else if (userSaidHasTechnique && !hasTechnique) {
                reaction = getRandomReaction(reactions.falseFlag);
            } else {
                reaction = getRandomReaction(reactions.incorrect);
            }
        }
        ...
        if (isCorrect) {
            ...
            // 技法識別コールバック（moderate/minimal では Phase1正解時に呼ぶ）
            if (feedbackLevel !== 'detailed' && hasTechnique && onTechniqueIdentified) {
                currentQuestion.techniques.forEach(t => onTechniqueIdentified(t));
            }
        } else {
            setStreakJustBroke(streak > 1);
            setStreak(0);
        }

        const reactionType = getReactionType(isCorrect, hasTechnique, userSaidHasTechnique);
        enterFeedback(answer, reactionType);
    };
    ...
    const handleRebuttalSubmit = () => {
        ...
        const answer = {
            ...
            wrongSelections,
            missedTechniques,
            responseTimeMs: firstDecisionTimeRef.current || 0,
        };
        ...
        const reaction = reactions ? getRandomReaction(reactions.correct) : null;
        setFeedbackReaction(reaction);
        setStreak(newStreak);
        setStreakJustBroke(false);
        setScore(prev => prev + points);

        // detailed: 反駁で正しく選んだ技法のみコールバック
        if (onTechniqueIdentified) {
            correctSelections.forEach(t => onTechniqueIdentified(t));
        }

        enterFeedback(answer, 'flaggedCorrectly');
    };

    const handleCriticismChoice = (choice) => {
        setCriticismChoice(choice);
        setCriticismPhase('responded');
        // スコアへの影響
        if (choice.score !== 0) {
            setScore(prev => Math.max(0, prev + choice.score));
        }
        // 倫理ボーナスをエンディングフラグに反映
        if (onEthicsChange && choice.ethicsBonus !== 0) {
            onEthicsChange(choice.ethicsBonus);
        }
        // 研究データ記録用コールバック
        if (onCriticismResponse) {
            onCriticismResponse({
                questionId: currentQuestion.id,
                criticismEventId: criticismEvent.id,
                choiceId: choice.id,
                scoreChange: choice.score,
                ethicsChange: choice.ethicsBonus,
            });
        }
    };
    ...
    const handleNext = () => {
        ...
        setPostReactionType(null);
        setPostReactionData(null);
        setNpcReplies(null);
        setStreakJustBroke(false);
        setCriticismEvent(null);
        setCriticismPhase('idle');
        setCriticismChoice(null);

        if (currentIndex < questions.length - 1) {
            questionStartTimeRef.current = Date.now();
            firstDecisionTimeRef.current = null;
            setCurrentIndex(prev => prev + 1);
        } else {
            const finalAnswers = [...answers, lastAnswer].filter(Boolean);
    ...
    const getFeedbackExplanation = (explanation) => {
        if (feedbackLevel === 'detailed') return explanation;
        if (feedbackLevel === 'moderate') return explanation.split('。')[0] + '。';
        return null;
    };
    ...
    return (
        <div className="min-h-screen p-4 flex flex-col">
            ...
            <div className="max-w-2xl w-full mx-auto mb-4">
                <div className={`glass rounded-2xl p-4 ${isBoss ? 'border-2 border-yellow-500/50' : ''}`}>
                    <div className="flex items-center justify-between mb-2">
                        <span className={`text-sm ${isBoss ? 'text-yellow-400 font-bold' : 'text-gray-400'}`}>
                            {isBoss && '⚔️ '}{stageLabel}
                        </span>
                    ...
                    <div className="h-2 bg-white/10 rounded-full overflow-hidden">
                        <div
                            className={`h-full rounded-full transition-all ${isBoss ? 'bg-yellow-500' : 'progress-bar'}`}
            ...
            <div className="max-w-2xl w-full mx-auto">
                <ComboSystem streak={streak} justBroke={streakJustBroke} />
            </div>
            ...
            {showTechniqueHelp && (
                ...
                >
                    <div
                        className="bg-slate-800 rounded-3xl p-6 max-w-2xl w-full max-h-[85vh] overflow-y-auto scrollbar-hide animate-fade-in"
                        onClick={e => e.stopPropagation()}
                    >
                        <div className="flex items-center justify-between mb-6">
                            <h2 className="text-xl font-bold flex items-center gap-2">
                                <span>📖</span>5つのレトリック技法
                            </h2>
                            <button
                                onClick={() => setShowTechniqueHelp(false)}
                        ...
                        <div className="space-y-4">
                            {Object.values(TECHNIQUES).map(tech => {
                                const icons = {
                                    fear: '😱', authority: '👔', fabricated_evidence: '🔬',
                ...
                <div className="max-w-2xl w-full">
                    ...
                    {phase === 'question' && (
                        <div className="animate-fade-in">
                            {getQuestionIntro(currentIndex) && (
                                <DialogueBox dialogue={getQuestionIntro(currentIndex)} />
                            )}
                    ...
                    {phase === 'feedback' && lastAnswer && (
                        <div className="animate-fade-in">
                            {feedbackReaction && <DialogueBox dialogue={feedbackReaction} />}
                            ...
                            {/* NPCリプライフィード */}
                            {npcReplies && npcReplies.length > 0 && (
                                <NPCReplyFeed replies={npcReplies} delayMs={500} />
                            )}

                            {/* 批判イベント: 選択フェーズ */}
                            {criticismEvent && criticismPhase === 'showing' && (
                                <div className="my-4 animate-fade-in">
                                    {criticismEvent.dialogue.map((d, i) => (
                                        <DialogueBox key={i} dialogue={d} />
                                    ))}
                                    <DialogueBox dialogue={criticismEvent.bossComment} />
                                    <div className="glass rounded-2xl p-5">
                                        <p className="text-sm text-gray-400 mb-3 font-medium">どうする？</p>
                                        <div className="space-y-2">
                                            {criticismEvent.choices.map(choice => (
                                                <button
                                                    key={choice.id}
                                                    onClick={() => handleCriticismChoice(choice)}
                                                    className="w-full text-left p-4 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 hover:border-indigo-500/50 transition-all text-sm font-medium"
                                                >
                                                    {choice.text}
                                                </button>
                                            ))}
                            ...
                            {/* 批判イベント: 選択後フィードバック */}
                            {criticismEvent && criticismPhase === 'responded' && criticismChoice && (
                                <div className="my-4 animate-fade-in">
                                    <DialogueBox dialogue={criticismChoice.feedback} />
                                    <div className="glass rounded-xl px-5 py-3 flex items-center gap-4 text-sm mb-2">
                                        <span className={criticismChoice.score >= 0 ? 'text-green-400 font-bold' : 'text-red-400 font-bold'}>
                                            {criticismChoice.score >= 0 ? `スコア +${criticismChoice.score}` : `スコア ${criticismChoice.score}`}
                                        </span>
                                        <span className="text-gray-600">|</span>
                                        <span className={criticismChoice.ethicsBonus >= 0 ? 'text-blue-400' : 'text-orange-400'}>
                                            倫理 {criticismChoice.ethicsBonus >= 0 ? `+${criticismChoice.ethicsBonus}` : criticismChoice.ethicsBonus}
                                        </span>
                                    </div>
                                </div>
                            )}

                            {/* フィードバックカード: 批判イベント処理後のみ表示 */}
                            {criticismPhase !== 'showing' && <div className={`glass rounded-3xl p-6 md:p-8 border-2 ${
                                ...
                                {/* 反駁結果（detailedのみ） */}
                                {rebuttalResult && feedbackLevel === 'detailed' && (
                                ...
                                <div className="bg-white/5 rounded-2xl p-5 mb-6">
                                    ...
                                    {getFeedbackExplanation(currentQuestion.explanation) && (
                                        <>
                                ...
                                >
                                    {currentIndex < questions.length - 1 ? '次の問題へ' : '完了'}
                                </button>
                            </div>}
                        </div>
                    )}
