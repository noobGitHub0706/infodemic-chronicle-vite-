...
export const Stage2Intro = ({ rhetoricSummaryData, onContinue }) => {
    return (
            ...
            <div className="max-w-2xl w-full animate-fade-in">
                <div className="glass rounded-3xl p-8 md:p-12">
                    ...
                    <div className="text-center mb-8">
                        ...
                        <h1 className="text-3xl font-bold mb-2">レトリック識別訓練 完了！</h1>
                        <p className="text-gray-400">3ステージの訓練を突破しました</p>
                    </div>

                    <div className="bg-white/5 rounded-2xl p-6 mb-8">
                        <div className="grid grid-cols-2 gap-4 mb-4">
                            <div className="text-center">
                                <p className="text-sm text-gray-400">平均正答率</p>
                                <p className={`text-3xl font-bold ${
                                    rhetoricSummaryData.accuracy >= 70 ? 'text-green-400' :
                                    rhetoricSummaryData.accuracy >= 50 ? 'text-yellow-400' : 'text-red-400'
                                }`}>
                                    {rhetoricSummaryData.accuracy}%
                                </p>
                            </div>
                            <div className="text-center">
                                <p className="text-sm text-gray-400">累計スコア</p>
                                <p className="text-3xl font-bold text-indigo-400">{rhetoricSummaryData.score}</p>
                            </div>
                        </div>

                        {rhetoricSummaryData.weakTechniques.length > 0 && (
                            <div className="border-t border-white/10 pt-4">
                                <p className="text-sm text-gray-400 mb-2">見破りにくかった技法:</p>
                                <div className="flex flex-wrap gap-2">
                                    {rhetoricSummaryData.weakTechniques.map(tech => (
                    ...
                    {/* 正答率への黒田コメント */}
                    <DialogueBox dialogue={getResultComment(rhetoricSummaryData.accuracy)} />
                    ...
                    {/* 第2章への橋渡しセリフ */}
                    {STAGE2INTRO_NARRATIVE.transition.map((dialogue, i) => (
