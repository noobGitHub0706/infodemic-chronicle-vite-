...
const PROBLEM_OPTIONS = [
    ...
    { id: 'scientific_veneer', label: '科学的に見せかけているが根拠が曖昧', icon: '🔬' },
    { id: 'testimonial', label: '個人の体験談を一般化している', icon: '💬' },
    { id: 'social_proof', label: '「みんながやっている」という同調圧力', icon: '🌊' },
    { id: 'cherry_picking', label: '都合の良いデータだけを選んで提示している', icon: '🍒' }
];

/**
 * 汎用レトリック識別訓練コンポーネント
 *
 * @param {Array} questions - シャッフル済みの問題配列（ラッパー側でshuffleすること）
 * @param {'detailed'|'moderate'|'minimal'} feedbackLevel
 *   - detailed: 現行Stage1と同一（反駁詳細 + explanation全文）
 *   - moderate: 技法タグ + explanation先頭文のみ
 *   - minimal: 技法タグのみ（explanationなし）
 * @param {Object} narrative - { stageStart, questionIntros[], correctReactions[], incorrectReactions[], correctNoTechniqueReactions[], falseFlagReactions[] }
 * @param {string} stageLabel - ヘッダー表示用ラベル（"Stage 1: 見破る" 等）
 * @param {1|2|3} stageNumber
 * @param {Function} onComplete - 完了時コールバック
 */
export const RhetoricStage = ({
    questions,
    ...
    const getQuestionIntro = (index) => {
        const intros = narrative.questionIntros;
        ...
        if (userSaidHasTechnique && hasTechnique && feedbackLevel === 'detailed') {
            ...
            if (isCorrect) {
                reaction = hasTechnique
                    ? getRandomReaction(narrative.correctReactions)
                    : getRandomReaction(narrative.correctNoTechniqueReactions);
            } else if (userSaidHasTechnique && !hasTechnique) {
                reaction = getRandomReaction(narrative.falseFlagReactions);
            } else {
                reaction = getRandomReaction(narrative.incorrectReactions);
            }
    ...
    const handleRebuttalSubmit = () => {
        ...
        setFeedbackReaction(getRandomReaction(narrative.correctReactions));
        setAnswers(prev => [...prev, answer]);
        setLastAnswer(answer);
        setPhase('feedback');
        setStreak(prev => prev + 1);
    ...
    const getFeedbackExplanation = (explanation) => {
        ...
        return null; // minimal: no explanation
    };
                ...
                <div className="max-w-2xl w-full animate-fade-in">
                    <div className="glass rounded-3xl p-6 md:p-8">
                        ...
                        <div className="text-center mb-4">
                            <span className="text-sm text-gray-400 bg-white/5 px-3 py-1 rounded-full">{stageLabel}</span>
                        </div>
                        <DialogueBox dialogue={narrative.stageStart} />
                        <button
                            onClick={() => setStageStartShown(true)}
            ...
            <div className="flex-1 flex items-center justify-center">
                <div className="max-w-2xl w-full">
                    ...
                    {phase === 'feedback' && lastAnswer && (
                        <div className="animate-fade-in">
                            <DialogueBox dialogue={feedbackReaction} />
                            <div className={`glass rounded-3xl p-6 md:p-8 border-2 ${
                                ...
                                >
                                    {currentIndex < questions.length - 1 ? '次の問題へ' : (stageNumber < 3 ? '結果を見る' : 'Stage完了')}
                                </button>
                            </div>
                        </div>
                    )}
